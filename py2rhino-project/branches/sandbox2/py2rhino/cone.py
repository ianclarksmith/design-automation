...
from py2rhino._cone_prop import _ConeProp
...
class Cone(_SurfaceRoot):
    ...
    def __init__(self, _rhino_id):
        ...
        self.defm = _ObjectRootDefm(_rhino_id, Cone, _rsf )
        self.eval = _SurfaceRootEval(_rhino_id, Cone, _rsf )
        self.func = _SurfaceRootFuncOorc(_rhino_id, Cone, _rsf )
        self.genr = _SurfaceRootGenr(_rhino_id, Cone, _rsf )
        self.grps = _ObjectRootGrps(_rhino_id, Cone, _rsf )
        self.modf = _SurfaceRootMdfy(_rhino_id, Cone, _rsf )
        self.mtrl = _ObjectRootMtrl(_rhino_id, Cone, _rsf )
        self.prop = _ConeProp(_rhino_id, Cone, _rsf )
        self.rndr = _ObjectRootRndr(_rhino_id, Cone, _rsf )
        self.stat = _ObjectRootStat(_rhino_id, Cone, _rsf )
        self.test = _SurfaceRootTest(_rhino_id, Cone, _rsf )
        self.trfm = _ObjectRootTrfm(_rhino_id, Cone, _rsf )
        self.util = _ObjectRootUtil(_rhino_id, Cone, _rsf )

    @classmethod
    def create(cls, base, plane, height1, height2, radius, cap=pythoncom.Empty):

        _rhino_id = _rsf.add_cone(base, plane, height1, height2, radius, cap)
