...
def add_cone(base, height, radius, cap=None):

    return _rsf.add_cone(base, height, radius, cap)

def add_cone_2(plane, height, radius, cap=None):

    return _rsf.add_cone_2(plane, height, radius, cap)

def add_cut_plane(objects, start_point, end_point, normal=None):

    return _rsf.add_cut_plane(objects, start_point, end_point, normal)

def add_cylinder(base, height, radius, cap=None):

    return _rsf.add_cylinder(base, height, radius, cap)

def add_cylinder_2(plane, height, radius, cap=None):
...
def add_loft_srf(objects, start_pt=None, end_pt=None, type=None, style=None, value=None, closed=None):
...
def add_rev_srf(profile, axis, start_angle=None, end_angle=None):
...
def add_srf_contour_crvs(object, start_point, end_point, interval=None):

    return _rsf.add_srf_contour_crvs(object, start_point, end_point, interval)

def add_srf_contour_crvs_2(object, plane, interval=None):

    return _rsf.add_srf_contour_crvs_2(object, plane, interval)

def add_srf_control_pt_grid(count, points, degree=None):
...
def add_srf_pt_grid(count, points, degree=None, closed=None):
...
def add_sweep_1(rail, shapes, start_pt=None, end_pt=None, closed=None, style=None, style_arg=None, simplify=None, simplify_arg=None):

    return _rsf.add_sweep_1(rail, shapes, start_pt, end_pt, closed, style, style_arg, simplify, simplify_arg)

def add_sweep_2(rails, shapes, start_pt=None, end_pt=None, closed=None, simple_sweep=None, maintain_height=None, simplify=None, simplify_arg=None):

    return _rsf.add_sweep_2(rails, shapes, start_pt, end_pt, closed, simple_sweep, maintain_height, simplify, simplify_arg)

def add_torus(base, major_radius, minor_radius, direction=None):
...
def duplicate_edge_curves(object, select=None):
...
def explode_polysurfaces(objects, delete=None):
...
def extrude_curve_tapered(curve, distance, direction, base_point, angle, corner_type=None):

    return _rsf.extrude_curve_tapered(curve, distance, direction, base_point, angle, corner_type)

def extrude_surface(surface, curve, cap=None):

    return _rsf.extrude_surface(surface, curve, cap)

def fit_surface(object, degree=None, tolerance=None):

    return _rsf.fit_surface(object, degree, tolerance)

def flip_surface(object, flip=None):

    return _rsf.flip_surface(object, flip)

def insert_surface_knot(object, parameter, direction, symmetrical=None):

    return _rsf.insert_surface_knot(object, parameter, direction, symmetrical)

def intersect_breps(brep_1, brep_2, tolerance=None):
...
def is_surface_planar(object, tolerance=None):
...
def join_surfaces(object, delete=None):

    return _rsf.join_surfaces(object, delete)

def make_surface_non_periodic(object, direction, delete=None):

    return _rsf.make_surface_non_periodic(object, direction, delete)

def make_surface_periodic(object, direction, delete=None):
...
def pull_curve(surface, curve, delete=None):

    return _rsf.pull_curve(surface, curve, delete)

def rebuild_surface(object, degree=None, point_count=None):
...
def split_brep(brep, cutter, delete=None):
...
def surface_contour_points(object, start_point, end_point, interval=None, angle=None):
...
def surface_degree(object, direction=None):
...
def surface_edit_points(object, return_parameters=None, return_all=None):
...
def surface_isocurve_density(object, density=None):
...
def surface_points(object, return_all=None):
...
def surface_surface_intersection(surface_a, surface_b, tolerance=None, create=None):
